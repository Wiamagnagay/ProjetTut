    ...
    if (typeAction === 'creation_complete') {
      const payload = {
        ...
        profilPro: {
          ...
          profession: formStore.profession.trim(),
          structure: formStore.structure.trim(),
          villeEtablissement: formStore.ville.trim(),
          zoneGeoPatient: formStore.zone.trim(),
          milieuProfessionnel: formStore.milieu.trim(),
          connaissanceRI2S: formStore.commentConnu,
          infoComplementaires: formStore.infosComplementairesPro
            ? formStore.infosComplementairesPro.trim()
            : '',
          participationExpe: 'OUI',
          emailPro: formStore.email.trim(),
          telephonePro: formStore.telephone.replace(/\s/g, '').trim(),
        },
        profilIndustriel: null,
        demandesExperimentations: [],
      };
      ...
      if (!response.ok) throw new Error('Erreur création complète Pro');

      const result = await response.json();
      formStore.idUtilisateurGenere = result.idUtilisateur || result.id;
    }

    else if (typeAction === 'ajout_profil_pro') {
      if (!idUtilisateur)
      ...
      const response = await fetch(
        `https://formulaire-ri2s-1.onrender.com/api/utilisateurs/${idUtilisateur}/profil-pro`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(payloadPro),
        }
      );
          ...
          <div class="twoCol">
            <div class="leftCol">
              <div class="field">
                ...
                <input
                  class="input small"
                  :class="{ 'input-error': errors.profession }"
                  v-model="formStore.profession"
                />
                <p v-if="errors.profession" class="error">
                  {{ errors.profession }}
                </p>
              </div>
              <div class="field">
                ...
                <input
                  ...
                  v-model="formStore.ville"
                />
                <p v-if="errors.ville" class="error">{{ errors.ville }}</p>
              </div>
              <div class="field">
                <label
                  >De quelle zone géographique sont issus vos patients ?
                  <span class="req">*</span></label
                >
                <input
                  class="input small"
                  :class="{ 'input-error': errors.zone }"
                  v-model="formStore.zone"
                />
              ...
              <div class="field">
                ...
                <input
                  class="input small"
                  :class="{ 'input-error': errors.milieu }"
                  v-model="formStore.milieu"
                />
              ...
              <div class="field">
                ...
                <input
                  class="input small"
                  :class="{ 'input-error': errors.structure }"
                  v-model="formStore.structure"
                />
                <p v-if="errors.structure" class="error">
                  {{ errors.structure }}
                </p>
              </div>
              <div class="field">
                ...
                <input
                  class="input small"
                  v-model="formStore.infosComplementairesPro"
                />
            ...
            <div class="rightCol">
              <div class="field">
                ...
                <label
                  ><input
                    type="radio"
                    value="structure_partenaire"
                    v-model="formStore.commentConnu"
                  />
                  Par l'intermédiaire d'une structure partenaire</label
                >
                <label
                  ><input
                    type="radio"
                    ...
                    v-model="formStore.commentConnu"
                  />
                  Vous nous suivez sur LinkedIn.</label
                >
                <label
                  ><input
                    type="radio"
                    ...
                    v-model="formStore.commentConnu"
                  />
                  Par l'intermédiaire d'un confrère.</label
                >
                <label
                  ><input
                    type="radio"
                    ...
                    v-model="formStore.commentConnu"
                  />
                  Par l'intermédiaire d'un de vos patients.</label
                >
                <label
                  ><input
                    type="radio"
                    ...
                    v-model="formStore.commentConnu"
                  />
                  Vous avez reçu un flyer, vu une affiche</label
                >
                <label
                  ><input
                    type="radio"
                    ...
                    v-model="formStore.commentConnu"
                  />
                  Autre (précisez à la question suivante)</label
                >
                <p v-if="errors.commentConnu" class="error">
                  {{ errors.commentConnu }}
                </p>
              </div>
            </div>
